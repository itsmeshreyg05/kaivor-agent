...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".kaivor_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".kaivor_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using KAIVOR profile")
                kaivor = str(Path.home() / ".kaivor_profiles" / "firefox_kaivor")
                Path(kaivor).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(kaivor, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".kaivor_profiles" / "safari")
        ...
        # automation). Fall back to a persistent KAIVOR automation profile —
        # accounts logged in here once stay logged in on later sessions too.
        kaivor_profile = str(Path.home() / ".kaivor_profiles" / self.browser_name)
        Path(kaivor_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with KAIVOR profile: {kaivor_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(kaivor_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with KAIVOR profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "kaivor_screenshot.png")

"""
dashboard/server.py — KAIVOR Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "KAIVOR Uploads",
        Path.home() / "Documents" / "KAIVOR Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'KAIVOR-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"KAIVOR Dashboard Port {port}"
        prog_rule  = "KAIVOR Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="kaivor_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until KAIVOR is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "kaivor.key").exists() and (certs / "kaivor.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in KAIVOR to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('kaivor_token','{tok}');
  sessionStorage.setItem('kaivor_key','{key}');
  localStorage.setItem('kaivor_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to KAIVOR…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "kaivor.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "kaivor.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in KAIVOR UI to get the QR code.")
        await uvicorn.Server(cfg).serve()

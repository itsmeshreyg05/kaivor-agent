...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "K.A.I.V.O.R", parent=None):
    ...
    def paintEvent(self, _):
        ...
        # KAIVOR's own voice while speaking). Falls back to a gentle idle
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "kaivor"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("kaivor:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for KAIVOR", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure K.A.I.V.O.R. before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="KAIVOR", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "KAIVOR"
...
class AudioDeviceOverlay(_HudOverlay):
    """Choose which microphone KAIVOR listens to and which speakers it uses.
    ...
    is plugged in. 'KAIVOR can't hear me' is usually 'KAIVOR is listening to the
    webcam'."""
    ...
    def __init__(self, parent=None):
        ...
        self._in_box  = _row("MICROPHONE — what KAIVOR hears you with",
                             "input", get_input_device())
        lay.addSpacing(4)
        self._out_box = _row("SPEAKERS — what KAIVOR talks through",
...
class MemoryOverlay(_HudOverlay):
    """Everything KAIVOR has stored about you, and when it learned it.
    ...
    def _rebuild(self):
        ...
        hdr = QLabel("🧠  WHAT KAIVOR REMEMBERS")
        hdr.setFont(QFont("Courier New", 12, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.PRI}; background: transparent;")
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Kaivor actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — KAIVOR ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "KAIVOR").strip()
        ...
        self.on_interrupt      = None   # callable: () -> None — stop KAIVOR mid-speech
        self.on_voice_change   = None   # callable: () -> None — rebuild session with new voice
        self.on_audio_device_change = None  # callable: () -> None — reopen audio streams
        self._confirm_overlay  = None   # live ConfirmBanner, if one is on screen
        self.get_plugins       = None   # callable: () -> list[dict], set by KaivorLive
        self.get_plugin_settings = None # callable: () -> list[dict] settings schemas, set by KaivorLive
        self.on_wake_toggle    = None   # callable: (enable: bool) -> str, set by KaivorLive
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_kaivor_icon(out_path: Path) -> bool:
        """
        Render a KAIVOR arc-reactor icon at 4× resolution and downsample
    ...
    def _create_lnk_windows(lnk: str, target: str, args: str,
        ...
        try:
            ...
            sc.Description      = "K.A.I.V.O.R AI Assistant"
    ...
    def _create_desktop_shortcut(self):
        ...
        ico_path = Path(__file__).resolve().parent / "config" / "kaivor.ico"
        if not ico_path.exists():
            self._build_kaivor_icon(ico_path)
        ...
        try:
            ...
            if _os == "Windows":
                ...
                lnk      = str(desktop / "K.A.I.V.O.R.lnk")
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>KAIVOR</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.kaivor.assistant</string>\n'
                    '  <key>CFBundleName</key><string>K.A.I.V.O.R</string>\n'
            ...
            # ── Linux — .desktop file (Terminal=false, no console) ────────────
                ...
                desk = desktop / "K.A.I.V.O.R.desktop"
                desk.write_text(
                    "[Desktop Entry]\n"
                    "Name=K.A.I.V.O.R\n"
                    f"Exec={python} {script}\n"
    ...
    def _build_header(self) -> QWidget:
        ...
        _sub_text = ("Just A Rather Very Intelligent System"
                     if _disp in ("KAIVOR", "K.A.I.V.O.R")
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("By Shrey Garg", C.PRI_DIM))
        return w
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "KAIVOR_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.kaivor.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "kaivor.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "KAIVOR_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "KAIVOR_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.kaivor.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.kaivor.assistant</string>\n'
    ...
    def _wake_state(self) -> dict:
        ...
        if self.wake_get_state:
            try:
                ...
                pass
        # Before KaivorLive has wired its callback (drawer built at startup).
    ...
    def _toggle_wake_word(self):
        ...
        if not st["ready"]:
            ...
            return
        # Already downloaded → just flip enabled/disabled through KaivorLive.
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "KAIVOR") or "KAIVOR",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = "",
        ...
        self._assistant_name = name.strip() or "KAIVOR"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — {APP_VERSION}")
        self._title_lbl.setText(display)
        if display in ("KAIVOR", "K.A.I.V.O.R"):
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "KAIVOR") or "KAIVOR"
